...
module.exports = () => {
  ...
  /*tweetsJSON = tweetsJSON.map((tweet, index) => tweet.created_at = Date.now() - (oneDayMs * (tweetsJSON.length - index))); - return an erro in console /vagrant/w4/tweeter/server/lib/date-adjust.js:16
  tweetsJSON = tweetsJSON.map((tweet, index) => tweet.created_at = Date.now() - (oneDayMs * (tweetsJSON.length - index)));
  ^

TypeError: Cannot create property 'created_at' on number '1605376912995'
at tweetsJSON.map (/vagrant/w4/tweeter/server/lib/date-adjust.js:16:66)

*/

  tweetsJSON = tweetsJSON.map((tweet, index) => tweet = Date.now() - (oneDayMs * (tweetsJSON.length - index)));
